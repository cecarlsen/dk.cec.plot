...
namespace PlotInternals
{
	public class Earcut
	{
		public static void Triangulate( Vector2[] outlinePoints, int outlinePointCount, IList<int> holeIndices, ref List<int> triangles )
		{
			...
			if( hasHoles ) outerNode = EliminateHoles( outlinePoints, outlinePointCount, holeIndices, outerNode );

			// if the shape is not too simple, we'll use z-order curve hash later; calculate polygon bbox
			if( outlinePointCount > 80 )
			{
		...
		static Node LinkedList( Vector2[] outlinePoints, int start, int end, bool clockwise )
		{
			Node last = default( Node );
			if( clockwise == ( SignedArea( outlinePoints, start, end ) > 0 ) ){
		...
		static Node EliminateHoles( Vector2[] outlinePoints, int outlinePointCount, IList<int> holeIndices, Node outerNode )
		{
			List<Node> queue = new List<Node>();

			int holeCount = holeIndices.Count;

			for( int i = 0; i < holeCount; i++ )
			{
				int start = holeIndices[i];
				int end = i < holeCount - 1 ? holeIndices[i + 1] : outlinePointCount;
				Node list = LinkedList( outlinePoints, start, end, false );
		...
		static float SignedArea( Vector2[] data, int start, int end )
		{
		...
		public static float Deviation( Vector2[] data, IList<int> holeIndices, IList<int> triangles )
		{
			...
			if( hasHoles )
			{
				...
				{
					int start = holeIndices[i];
					int end = i < len - 1 ? holeIndices[i + 1] : data.Length;
			...
			{
				Vector2 va = data[ triangles[ i ] ];
				Vector2 vb = data[ triangles[ i+1 ] ];
				Vector2 vc = data[ triangles[ i+2 ] ];
